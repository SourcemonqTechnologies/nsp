...
#  district aishe code
@app.route('/AISHE_district',methods =['POST' , 'GET'])
def district():
    _json =request.json
    _institute_category =_json['institute_category']
    _state = _json['state']
    _district = _json['district']
    _name = _json['name']

    if  _institute_category and _state and _district and _name and  request.method =='POST':
        
        
        id= monq.db.institute_district_aishe.insert_one({'category':_institute_category,
                                            'state':_state,
                                           'district' :_district,
                                           "name" : _name})
...
def AIS1():
        d1 = monq.db.institute_district_aishe.find()
        resp =dumps(d1)
        return resp
...
def AIS2():
    user = monq.db.institute_district_aishe.find({'_id':ObjectId(id)})
    resp = dumps(user)
    return resp
...
def delete(id):
    monq.db.institute_district_aishe.delete_one({'_id':ObjectId(id)})
...
def institi():
    ...
    if  _institute_type and _state and _district and _university_type and  request.method =='POST':
        ...
        id= monq.db.instite_aishe.insert_one({'instiute_type':_institute_type,
                                            'state':_state,
                                           'district' :_district,
                                           "university_type" : _university_type})
